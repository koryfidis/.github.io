// plot_hists.C – ROOT macro with correct entry counting
#include <TFile.h>
#include <TH1D.h>
#include <TCanvas.h>
#include <fstream>
...
void plot_hists() {
    // ---------------------------------------------------------------
    // 1. Load theta histogram (theta_hit_bins.txt)
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // 2. Load phi histogram (phi_hit_bins.txt)
    // ---------------------------------------------------------------
    ...
    // Save to file
    TFile out_file("histograms.root", "RECREATE");
    h_theta->Write();
    h_phi->Write();
    out_file.Close();
}
